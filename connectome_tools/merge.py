...
from connectome_tools.utils import load_yaml, setup_logging
...
class CreatePartialRecipe:
    ...
    def checksum(self):
        ...
        params = (
            str(self.circuit_config.resolve()),
            self.strategies,
            self.seed,
        )
    ...
    def run(self):
        ...
        s2f_recipe.main(
            circuit=str(self.circuit_config),
...
class CreateFullRecipe:
    ...
    def run(self):
        ...
        config = load_yaml(self.main_config)
        ...
        all_tasks = [
            ...
            for i, params in enumerate(config["regions"])
        ]
        ...
        if pending_tasks:
            executor_config = load_yaml(self.executor_config)
            folder = self._slurm_path / f"{datetime.now():%Y%m%dT%H%M%S}"
            L.info("Slurm folder: %s", folder)
            failures = execute_pending_tasks(pending_tasks, executor_config["executor"], folder)
            if failures:
        ...
        tree = _build_xml_tree(
            ...
            comment=(
                ...
                f"\nfrom circuit {self.circuit_config}"
